...
class TestGateToCircuit:
    def test_to_clifford_plus_t(self, snapshot):
        snapshot.assert_match(
            repr(GatesCircuit.from_qasm(EXAMPLE_CIRCUIT_NO_APPROX_NEEDED).to_clifford_plus_t()),
            "circuit_no_approx.txt",
        )

        snapshot.assert_match(
            repr(GatesCircuit.from_qasm(EXAMPLE_CIRCUIT_NEEDING_APPROX).to_clifford_plus_t()),
